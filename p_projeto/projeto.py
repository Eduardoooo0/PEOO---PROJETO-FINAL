...
class App:
    def __init__(self, root):
      try:
        ...
        self.l_lancamentos = tk.Label(self.root)
        ft = tkFont.Font(family='Times', size=33)
        self.l_lancamentos["bg"] = "red"
        self.l_lancamentos["font"] = ft
        self.l_lancamentos["fg"] = "white"
        self.l_lancamentos["justify"] = "center"
        self.l_lancamentos["text"] = "Lançamentos"
        ...
        self.conta = tk.Button(self.root)
        self.conta["activebackground"] = "red"
        self.conta["bg"] = "black"
        self.conta["borderwidth"] = "0px"
        self.ft = tkFont.Font(family='Times', size=14)
        self.conta["font"] = self.ft
        self.conta["fg"] = "white"
        self.conta["justify"] = "center"
        self.conta["text"] = "Minha conta"
        self.conta["relief"] = "raised"
        self.conta.place(x=1240, y=20, width=100, height=25)
        self.conta["command"] = self.criar_entrar_conta   
      except:
        messagebox.showwarning('ERRO','ERRO DESCONHECIDO')
    ...
    def entra_em_conta(self):
        try:
          ...
          if self.verificar_nome_existente(nome):
              self.conta["text"] = nome
              self.conta.place(x=1240, y=20, width=100, height=25)
              self.conta["command"] = self.b_da_Conta
              self.Sair(self.canva3)
          else:
              self.adicionar_pessoa(nome, senha)
          self.segundos = 0
          while self.segundos < 1:
              time.sleep(0.1)
              self.segundos += 0.1
          self.canva3.destroy()
        except:
    ...
    def verificar_nome_existente(self, nome):
        try:
            self.arquivo = open("pessoas.bin", "rb")
            self.pessoas = pickle.load(self.arquivo)
    ...
    def adicionar_pessoa(self, nome, senha):
        self.pessoa = {"nome": nome, "senha": senha}
        try:
            self.arquivo = open("pessoas.bin", "rb")
            self.pessoas = pickle.load(self.arquivo)
        except FileNotFoundError:
        ...
        self.pessoas.append(self.pessoa)

        try:
            self.arquivo = open("pessoas.bin", "wb")
            pickle.dump(self.pessoas, self.arquivo)
        finally:
            self.arquivo.close()
    ...
    def Ok(self, quant):
        try:
          if int(quant) > 0:
            ...
            messagebox.showwarning('ERRO','DIGITE UMA QUANTIDADE MAIOR QUE ZERO')
        except ValueError:
             messagebox.showwarning('ERRO','DIGITE APENAS NÚMEROS')
    def Finalizar(self, canva):
        messagebox.showinfo("COMPRA","Compra finalizada")
        self.segundos = 0
        while self.segundos < 1:
            time.sleep(0.1)
            self.segundos += 0.1
